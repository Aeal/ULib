...
public class SoundManager : MonoBehaviour
{
    ...
    public void Start()
    {
        Debug.Log("Starting sound manager");
        instance = this;
        Instance.Init(true, true, 2, 6, "Audio");
    ...
    * 2D sound functions
    */
    public static AudioSource Play2DSound(string name, float volume = .5f,string library = "default", bool loop = false,bool fadeMusic = false, float fadeTo = 0, float fadeTime = 0)
    {
        AudioSource channel = null; 
        try
        {
            channel = Instance.GetFirstUnusedSoundChannel();
    ...
    public static AudioSource Play2DSoudWithCallback(string name, Delegate callBack, float volume = .5f, string library = "default", bool loop = false, bool fadeMusic = false, float fadeTo = 0, float fadeTime = 0)
    {
        AudioSource channel = null; 
    ...
    public static AudioSource Play2DSoundQueue(string[] names, float volume = .5f, string library = "default", bool loop = false)
    {
        Debug.Log(names);
        AudioSource channel = Instance.gameObject.AddComponent<AudioSource>(); 

        Instance.soundQueues2D.Add(new SoundQueue(channel, names, library,loop,volume));
    ...
    */
    ...
    public static AudioSource Play3DSoundWithCallback(GameObject source, string soundName, Delegate callBack, string library = "default", bool loop = false, float volume = .5f)
    {
    ...
    public static AudioSource Play3DSoundWithCallback(GameObject source, AudioClip clip, Delegate callBack, string library = "default", bool loop = false, float volume = .5f)
    {
        Debug.Log("Creating 3d audio callback");
        AudioSource audioSource = null; //= source.GetComponent<AudioSource>();
    ...
    public static AudioSource Play3DSound(GameObject source, string soundName, string library = "default", bool loop = false, float volume = .5f)
    {
        AudioSource audioSource = source.GetComponent<AudioSource>();
    ...
    public static AudioSource Play3DSoundQueue(string[] names, GameObject source, float volume = .5f, string library = "default", bool loop = false)
    {
        Debug.Log(names);
        AudioSource channel = source.AddComponent<AudioSource>();
        Instance.soundQueues3D.Add(new SoundQueue(channel, names, library, loop,volume));
    ...
    #region Music
        ...
        {
            AudioSource scr = Instance.GetMusicChannelPlaying(name);
            scr.Pause();
        }
    ...
    private void LoadSounds(string Library = "default", string Directory = "Effects")
    {
        ...
        {
            if(!soundLibrary[Library].ContainsKey(o.name.ToLower()))
    ...
    private void LoadMusic()
    {
        ...
        foreach (Object o in music)
        {
            ...
            if(!musicFiles.ContainsKey(o.name.ToLower()))
    ...
    private void Update3DCallbacks()
    {
        ...
        for (int i = 0; i < callback3DAudioSource.Count; i++ )
        {
                AudioSource pair = callback3DAudioSource.Keys.ToArray()[i];
                if (pair.isPlaying) continue;
                callback3DAudioSource[pair].DynamicInvoke();
                callback3DAudioSource.Remove(pair);
                i--;
        }
    }
